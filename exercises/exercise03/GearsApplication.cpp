...
void GearsApplication::Update()
{
    ...
    float nearness = static_cast<float>(std::numbers::pi) * 0.5f;
    bool  perspective = true;
    if (perspective) {
        m_camera.SetPerspectiveProjectionMatrix(nearness, aspectRatio, 0.1f, 10.0f);
    }
    else {
...
void GearsApplication::Render()
{
    ...
    glm::mat4 leftGearMatrix(glm::translate(glm::vec3(-1.0f, 1.0f, 0.0f)) * glm::rotate(speed * time * leftRatio - 0.05f, axis) * glm::scale(glm::vec3(7.5)));

    DrawGear(m_smallGear, leftGearMatrix, Color(0.3f, 0.4f, 0.8f));
